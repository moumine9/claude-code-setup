...
description: Switch to the Haiku model (default effort medium)
argument-hint: "[low|medium|high|xhigh|max]"
---

## Ta tache

Bascule la session sur le modele **haiku**.

L'effort vient de `$ARGUMENTS` s'il est fourni, sinon `medium`.

```bash
bash /d/claude-code-setup/scripts/set-model.sh haiku "$ARGUMENTS"
```
